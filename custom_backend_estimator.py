...
class CustomBackendEstimator(BaseEstimator[PrimitiveJob[EstimatorResult]]):
    ...
    def _postprocessing(
        ...
        for i, j in zip(accum, accum[1:]):
            ...
            for k in range(i, j):
                ...
                # vsilva
                if self._run_options.get('in_range') != None:
                  in_range  = self._run_options['in_range']
                  shots     = sum(count.values())
                  # Get mitigated counts with contrast range
                  count     = mitigation.get_counts(count, shots, in_range = in_range)
               
                expvals, variances = _pauli_expval_with_variance(count, paulis)
                
                #print ('shots:' + str(shots) + ' expvals=' + str(expvals) + ' variances=' + str (variances) + ' Paulis:' + str(paulis))
                #print ('shots:' + str(shots) + ' expvals1=' + str(expvals1) + ' variances1=' + str (variances1) + ' Paulis:' + str(paulis))
